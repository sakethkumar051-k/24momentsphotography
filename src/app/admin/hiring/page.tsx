...
import type { JobPosting, JobApplication } from '@/types/database';
        ...
        <div className="flex gap-2 border-b border-admin-border">
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
              tab === 'jobs' ? 'border-admin-gold text-admin-gold' : 'border-transparent text-admin-muted hover:text-admin-text'
            }`}
          >
            Job Postings
          </button>
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
              tab === 'applications' ? 'border-admin-gold text-admin-gold' : 'border-transparent text-admin-muted hover:text-admin-text'
            }`}
          >
                  ...
                  <div key={job.id} className="bg-admin-surface border border-admin-border rounded-xl p-6">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex gap-2 shrink-0">
                        ...
                        <button onClick={() => handleDeleteJob(job.id)} className="text-xs px-3 py-1.5 border border-red-200 rounded-lg hover:bg-red-50 text-red-500">
                          Delete
                        </button>
                      </div>
                    </div>
          ...
          applications.length === 0 ? (
            ...
            <div className="space-y-4">
              {applications.map((app) => (
                <div key={app.id} className="bg-admin-surface border border-admin-border rounded-xl p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-admin-muted mt-1">
                        {app.email}
                        {app.phone && ` · ${app.phone}`}
                        {' · '}Applied for <span className="font-medium text-admin-gold">{app.job_title}</span>
                        {' · '}{new Date(app.created_at).toLocaleDateString()}
                      </p>
                      {app.portfolio_url && (
                        <a href={app.portfolio_url} target="_blank" rel="noopener noreferrer" className="text-xs text-admin-gold hover:underline mt-1 inline-block">
                          {app.portfolio_url}
                        </a>
                      )}
                      {app.message && (
                        <p className="text-sm text-admin-muted mt-2 whitespace-pre-line">{app.message}</p>
                      )}
                    </div>
                    <button onClick={() => handleDeleteApp(app.id)} className="text-xs px-3 py-1.5 border border-red-200 rounded-lg hover:bg-red-50 text-red-500 shrink-0">
                      Delete
                    </button>
                  </div>
                </div>
